...
The Poison valid-text numeric/parser path is now read and conditionally executed
(`MAGIC-POISONINPUT-157`); its CString and decimal-library behavior is substituted,
so this is not a native parser observation. `MAGIC-EFFECT-015`'s separate Stone
Curse numeric-conversion frontier remains outside that population.
