...
stores and [`trigger.md`](trigger.md) what the evaluator does with the compiled
...
- Mission-entry range 004d422e..004d42be computes trunc(max(5, sqrt(n)+4)) from
  the flat unit-list count n. The referenced doubles 0059bc50/0059bc58 are
  4.0/5.0.
- Caller 004d4373 passes the computed radius low byte, r=R&255. A controlled
  n=63504 computes R=256 but calls placement with r=0.
- Primitive 004f4604 makes at most floor(r*r/2)+2 random attempts. Each attempt
  draws y before x through the inclusive 00504003, and each offset equals
  -floor(r/2)+draw(0..r). At odd r=5 the sampled offsets are -2..+3; the
  fallback instead scans -2..+2, x outer and y inner.
- Caller 004d4346..004d438a tries the hero at radius 0 and keeps a success. A
...
| ID | Claim | Confidence | Status | Evidence |
|---|---|---|---|---|
| MISSION-DEFEAT-045 | Primary fall, scripted outcome and automatic recovery are separate ordered branches of reporter `004d8963`; campaign maps load mode zero and cannot take the recovery branch. | High | ● active | [EXP-0274](../experiments/EXP-0274-defeat-modes/) |
...
- Reporter `004d8963` first requires player `+0x3d != 0` and `+0x28 == 0`.
- For unsigned latch byte `+0x3c >= 2`, only actual mode `server+0x0c != 0`,
  ...
  `004d3755` repair at `004d8a67`, then `004d403c` placement at `004d8a73`, then
...
- Below latch 2, a null primary or a nonzero primary stage `+0x13c` writes latch
  ...
  then calls `004fae9e`, which sets HP to -50 for every actor in the flat
  ownership index.
...
- `0xb4 -> client 00410623 -> window 0x433/0xff -> 0x431` constructs `00446de2`,
  stores the panel at `frontend+0x110` and shows it via `00476810`.
- Constructor vtable `00598b78` has `+0x48 = 00447063`, a forwarder to
  `004c52f3`. That base accepts `0x445/0x446`, stores the result through slot
  `+0x84 = 004c52b0`, and posts `0x44c`.
- Close `004757b0` matches the actual `+0x110` pointer. Result
  `0x445 -> 0x41e -> 00479dd0` teardown -> `0x421` -> `004769c0` main menu when
  the screen word is zero. Any other result -> `0x418`, the save-selection
  panel.
- A later successful selection `0x419` takes the campaign load path `00478af0`,
  not the dead-primary repair helper.
- Draw/init slot `+0x78 = 00446e22` uses title main[141], dialogs[44] for
...
- Load is disabled when `0043c0ae` finds no `game*.sav`.
- The neighboring vtable's handler `00446d35` does post `0x41d`, but it is not
  this panel's.
...
- `FUN_004d403c`, the routine that puts the player on the map, never reads the
  type-6 array. It iterates the player's own unit list at `player+0x20`
  (`004d42f7 CALL 0x00518160`, `004d44e8 CALL 0x005181d0`) and calls the
  placement routine `FUN_004f4604` on each.
- The hero (`player+0x34`, tested at `004d4353`) goes to the exact drop cell
  with radius `0`. Everything else goes to the same cell with a computed radius
  `ftol(max(5.0, sqrt(nUnits) + [0x0059bc50]))` (`004d423c`…`004d42be`). The
...
- The type-6 spawner `FUN_004e26bb` installs each record into its owner's own
  containers (`004e2f23`, `004e2fec`, `PARTY-WRITE-011`), and the placement walk
...
`FUN_004d403c`'s three callers are enumerated (`EXP-0100`).
...
- `FUN_004d403c`'s `this` is the server singleton `[0x005cd758]`, not a map
  object. Its caller `FUN_004d303e` passes its own `this` (`004d334d`); that
  `this` carries the mission number at `+0x80` and `SESS-LOAD-009`'s three
  sub-objects at `+0x14`, and it is reached with `ECX = [0x005cd758]` at
  `00477dc8`.
- The gate is `server+0x0c` and the array is `server+0x6c`, the same memory as
  `(server+0x44)+0x28`, since `server+0x44` is the map sub-object `FUN_004e1924`
  is invoked on.
- Before the array is consulted, `player+0x60` is a per-player override, gated
  on `server+0x0c` being nonzero (`004d4136`…`004d4147`); when it is taken the
  array is not read. `server+0x0c` is `arg0 < 2` from the server's constructor
  (`004cfe46`/`004cfe4a`/`004cfe53`), and the campaign start passes 2. On the
  ...
  `MISSION-DROP-011`).
- The index is `FUN_00504003(GetSize() - 1)` (`004d4187`…`004d4190`), a uniform
...
- The zero-coordinate fallback is `FUN_00504003(0x46) + 0x1e` on each axis
  independently. `FUN_00504003(n)` is `(rand() * (n+1)) / 32768`
  (`00504010`…`00504027`, `CDQ`/`AND EDX,0x7fff`/`SAR EAX,0xf`) and is therefore
...
- Being authored arms it, not being referenced. `FUN_004e3591`'s second pass
  walks every check node and gives each one the next slot in list order whether
  a trigger names it or not (`004e3591` decompile, `local_a0` incremented in
...
- `FUN_00473110` formats `"battle\m%d\event%02d"` (`0x005be58c`) from the
  mission number and the message number, prefixes `"main\text\"` (`0x005be580`)
  and appends `".txt"` (`0x005be578`), in three calls at `00473915`, `0047393d`,
  `00473956`. It then opens the result at `0047396c`.
...
- The failure path of `FUN_00473110` (EXP-0098) does nothing at all: no window,
...
- `FUN_004e26bb`: `CMP local_34[2],0x1a` splits the record set first.
  `+0x08 >= 0x1a` builds a `Unit` (`new(0x198)`, `FUN_004f2cb8`) out of the
  Units collection of `world.res:data/data.bin`, keyed on parameters
  `0x1d`/`0x1e`. Everything below builds a `Human` (`new(0x1e8)`,
  `FUN_004f8e78`) out of the Humans collection.
...
- The NPC arm reaches a definition through `scenario.res::npc.reg`:
  `FUN_004c86a0(0x5f0758, +0x0a)` fetches the npc record and `FUN_0048cbe0`
...
- `FUN_004de63e(id)` walks the collection at `defs+0xa0` from `GetCount()-1`
  downward (`004de650`…`004de655`). The loop stops at `> 0`, so entry 0 is never
  compared (`004de66a JLE`).
- It compares parameter `0x18`, the Humans `serverID` column, against the
  argument (`004de66c PUSH 0x18`, `004de68d CMP`) and falls out to `XOR EAX,EAX`
  (`004de698`) on a miss.
- A consumer that treats the return as "index or error" silently places entry 0
  for every unresolved id.
- `FUN_0048cbe0` reads `npc+0x14` and returns it unless it equals `0x1a`. In
  that case it composes the template from the npc's own `Flags` tokens: `"Mage"`
  `0x5bf270`, `"Female"` `0x5bf268`, `"MySex"` `0x5bf260`, `"MyClass"`
  `0x5bf258`, each tested by `FUN_0048c3d0` (`0048cc0b`…`0048ccae`).
...
- `FUN_004e3591`'s second pass assigns `local_a0`, reset to 0 at the head of the
  pass, to each check node in list order. It increments it for both a runtime
  check and a `0x10002` constant (`004e3591` decompile: the two increments and
...
- `SAV-711`: a resolution-rejected normal check does not advance the shared
  cursor `004e3591` keeps at `[EBP-0x9c]` (`local_a0` above).
  `004e4940 CMP [EBP-0x1ac],0x1` / `JNZ 004e4987` gates the increment at
  `004e4976`, so a validation failure shifts every later assignment down by one
  slot.
...
- Instrument: `EnumRefs disp:60`, whole image, 602 hits over 318 owners, 2 in
  orphan code: `004947b3 LEA EDI,[ESP+0x60]` and `004949d9 MOV EAX,[EBX+0x60]`,
  both in the `0x49xxxx` region, far from any `Player` method, one of them a
  stack displacement.
- Reduced by the field's own `u16` width, per the rule EXP-0066 set: 42 of the
  602 are byte- or word-wide, and five are word-wide. They are three reads in
  `FUN_004d403c` (`004d4141`, `004d414e`, `004d4160`), the constructor's zero
  (`004facbe`, `FUN_004faa81`) and `004d6e1b MOV word ptr [ECX + 0x60],BX` in
  `FUN_004d5dd8`.
- Not one of the 37 byte-wide hits is on a `Player`. The only two that store
  (`004d2139`/`004d2142`, `FUN_004d2105`) write an object whose `+0x54` was set
  to `0xe` two instructions earlier and whose `+0x61` is the next field; a
  `Player`'s `+0x64` is a `CString`.
- No dword hit at displacement `0x60` lies in `004fa000..004fd000` or
  `00510000..00512000`, the ranges the `Player` methods occupy. A dword store
  there would have to span `+0x60` and `+0x64`.
- The writing arm (`SESS-CMD-016`'s `0x32`, at `004d6d7f`) resolves an actor
...
- If neither phase finds a cell the routine returns 0 without committing, and
  ...
  (`0x5c61b8`) for any member, although the string names only the hero. On
...
- The drop cell applies to all members, not one, and the box grows with the
  ...
  log line and no other consequence. Neither the walk nor `FUN_004d303e` tests
...
- `004d8963` requires `player+0x3d != 0` and `player+0x28 == 0`.
...
- All four writers are increments, never stores:
  - instant arm 4, `00539d06 INC [EBP + 0xb3ac]`;
  - instant arm 5, `00539d11 INC [EBP + 0xb3b4]`;
  - check arm 18, `0053be5d INC [ECX + 0xb3b4]` (`FUN_0053be50`, gated on the
    unit's state being `0x10`);
  - a fourth in orphan code: `005386cc` load, `005386d3 INC EAX`, `005386da`
    store, of `+0xb3ac`, inside `ORPHAN[005384c8..005386e6]`.
- The only routine that clears either is `FUN_0052c0f0` (`0052c169`,
  `0052c175`), whose callers are three constructors (`EnumRefs callto:52c0f0`: 3
...
table at `005386ec` has 0 references by immediate and 0 by displacement
...
- Constructor `00446de2` installs vtable `00598b78`; slot `+0x48` contains
  `00447063`, forwarding the base close handler.
- Handler `00446d35` belongs to the neighboring class (slot `00598b38`).
- The actual close dispatcher `004757b0` matches `+0x110`, then posts `0x41e`
  for result `0x445`, otherwise `0x418`. Exit takes teardown
  `00479dd0 -> 0x421`; with screen word zero, `004769c0` opens the main menu.
...
- Stepper `004d2551` requires `server+0x2c != 0`. Start `004d00e9` sets it, and
  teardown `004d05b2` clears it and both clocks. Construction and `004d0cb7`
  deserialization also write this field.
- Campaign idle `00471600` skips the stepper while phase `frontend+0x6bc == 2`
  and screen mask `+0x3dc & 0x4008 != 0`. Both outcome panels use `00476810`,
...
**Confidence.** Medium. The census is exhaustive and byte-exact over both
...
1. Session command opcode `0x4`, the byte at `cmd+0x9`, reaches the arm at
   `004d828a`, which calls `FUN_004d303e` (`004d82cb`, `callto:` 1 hit / 1
   owner). The opcode is resolved by brute force over `FUN_004d5dd8`'s own index
   table: `00410588`-style dispatch at `004d6628`…`004d6663`, byte table
   `0x004d876e`, arm table `0x004d86fa`, both roots identical on all 189
   entries.
2. `FUN_004d303e` is `MISSION-DROP-002`'s routine: it runs `MISSION-START-001`'s
   placement walk `FUN_004d403c` (`004d3350`).
3. Immediately after the walk, gated on the player having a hero
   (`004d33fc CMP dword ptr [ECX + 0x34],0x0`, `MISSION-START-001`'s
   `player+0x34`), it fills the static message object at `0x00609c38`, the
   buffer `DLG-PATH-002` names. It writes opcode `0xab`
   (`004d340c MOV byte ptr [EDX + 0x9],0xab`), the player's slot at `+0x7`
   (`004d341a`), and the hero's own cell at `+0xa` and `+0xe`, read by
   `FUN_00544a10`/`FUN_00544a20` (`004d3427`, `004d3440`), the pair
   `MISSION-DROP-011` names for reading an actor's cell as two bytes. It then
   dispatches the message (`004d3459`).
4. On the client, `FUN_004104e8`'s arm for opcode `0xab` and for no other value
   (byte table `0x0041879b`, arm table `0x004186e3`, 189 entries, both roots
   identical) computes `ScrollTo(pkt[+0xa] − visCols/2, pkt[+0xe] − visRows/2)`
   (`0041841c`…`0041844a`). It is gated on `campaign+0x3c8 == 0` (`0041840d`);
   mission entry sets that field from its own argument at `00477e0b`. This is
...
(`EnumRefs re:.*0xab$`, 2 hits / 2 owners / 0 orphan). The second, `004d8ea6` in
`FUN_004d8dcc`, builds the same message on the `server+0x00 % 5 == 0` sub-tick
from a unit's cell, and no instruction in that routine hands it to a dispatcher.
Its constructor chain `FUN_004efd9e`→`FUN_004efa7b` only zeroes fields and
installs vtables `0x0059c1b8`/`0x0059c128`, so there is no self-enqueue either,
...
because the command queue between `FUN_004d303e` and the client arm was not
followed. Medium that `FUN_004d8dcc`'s message is inert: the absence of a
dispatch is read from that routine and its two constructors, an absence over
three routines and not over the image.

### MISSION-VIEW-020

- `FUN_00477c00` is `SESS-LOAD-009`'s only campaign caller of `FUN_004d00e9`
  (`00477dce`) and the routine that clears the view's landscape pointer on the
  way in (`00477c2e`). It later reads both keys through the `.reg` int getter
  `FUN_004ccc20` and stores the results straight onto the view
  (`0047813a`…`00478189`).
- `FUN_00477650` does the same (`0047784d`…`004778a1`), and `FUN_00478c40`
  writes the pair back out (`00478e0e`…`00478e3d`).
- The same store carries `CurrentState/InBattle`, `Character/Name`,
  `GameOptions/*`, `Objects/Group%d`, `Inventory/IsOpen`, `SpellBook/IsOpen`,
  `Projectiles/*` and `Fog/*` (`StrDump fnstr:477c00`). It is the session's own
...
- The restore is a no-op when the key is absent: the default handed to the
  ...
  (`004ccca7`). The pair can only carry a previously saved origin forward.
- Two entries later the same routine restores the two panels (`004781ae`,
  `004781c4`), which re-enters `FUN_0041e06d`, recomputes `visRows` and
...
- `FUN_00486d00` forks on the element's kind and builds the path with one
  `sprintf` each: kind 0 → `graphics\interface\Docs\%d.bmp` (`0x005bf04c`), kind
  1 → `main\text\Docs\%d.txt` (`0x005bf06c`). The first component names the
  archive.
- Both resolve exactly. `main.res` ships `text/docs/1.txt`…`4.txt` and
  ...
  unresolved value.
- A text document is read whole by `FUN_004682d0` (open, read the node's whole
  length, append one NUL, assign to element `+0x0c`) and wrapped by
  `DLG-WRAP-009`'s `FUN_00456ab0(element+0x24, text)` into the string array at
  `+0x10`. A picture is loaded into `+0x08` with no layout.
- Lifetime is the whole campaign, and the record is in the save file.
  `FUN_00489270` writes the collection as `u32 count` then `count` pairs of
  dwords; `FUN_00486ca0`/`FUN_00486cd0` push exactly `(this,4)` and
  `(this+4,4)`, the value and the kind, and nothing else. It runs with
  `ECX = app+0x548`, the campaign record, called at `00479411` from
  `FUN_00478c40`, whose string operands are the save tail store's own keys
  (`Fog/Data`, `Projectiles/*`, `View/*`, `Character/Name`).
- `FUN_00489580` reads the collection back from `FUN_00477650` and
  `FUN_00477c00`, the two mission-entry routines (`MISSION-VIEW-020`).
...
- `FUN_004879f0` loads `[Mission<n>] Payment` into mission-record `+0x0c`. The
  mission-completion path passes that field to `FUN_0041dacf`, while mission
...
Medium for interpreting the value as a purse credit: `FUN_0041dacf` was not read
...
- The new-campaign state machine constructs the session, resets the campaign
  record, and calls `FUN_00488460(10)`. Its loader appends
...
- Button activation posts `0x47a` with `button+0x70`. Success handler
  `FUN_004477a5` delegates to `FUN_0043c16c`, which closes the panel with
...
- Phases 0, 1 and 3 construct Change Map; phase 2 constructs Victory; all have
  result `0x41d`.
- Immediately after adding that row, `0043c7ad..0043c7c5` calls its
...
- `Player+0x3c` is stored at `00511137` and loaded at `005112a1`, and the
...
- `FUN_00477650` resets the latch at `00477a73`, and resumed mission entry
  resets it at `00478965`.
- The load-game path has already set campaign phase 2 at `00478b4c`, so the
